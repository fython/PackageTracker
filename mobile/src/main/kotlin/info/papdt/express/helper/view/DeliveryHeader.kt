...
class DeliveryHeader: View, RefreshHeader {

	private var mPaint: Paint = Paint()
	private var mCloudX1: Int = 0
	private var mCloudX2: Int = 0
	private var mCloudX3: Int = 0
	private var mHeaderHeight: Int = 0
	private var mAppreciation: Float = 0.toFloat()
	private var mState: RefreshState? = null
	private var mCloudDrawable: PathsDrawable? = null
	private var mUmbrellaDrawable: PathsDrawable? = null
	private var mBoxDrawable: PathsDrawable? = null
	...
	private fun initView(context: Context, attrs: AttributeSet?) {
		...
		minimumHeight = DensityUtil.dp2px(120f)

		mCloudDrawable = PathsDrawable()
		...
		mCloudDrawable!!.parserColors(0xffffffff.toInt())
		mCloudDrawable!!.setGeometricHeight(DensityUtil.dp2px(20f))
		mUmbrellaDrawable = PathsDrawable()
		...
		mUmbrellaDrawable!!.setGeometricWidth(DensityUtil.dp2px(200f))
		mBoxDrawable = PathsDrawable()
		...
		mBoxDrawable!!.setGeometricWidth(DensityUtil.dp2px(50f))
		...
		if (isInEditMode) {
			...
			mAppreciation = 100f
			mCloudX1 = (mCloudDrawable!!.width() * 3.5f).toInt()
			mCloudX2 = (mCloudDrawable!!.width() * 0.5f).toInt()
			mCloudX3 = (mCloudDrawable!!.width() * 2.0f).toInt()
		}
	}
	...
	override fun onDraw(canvas: Canvas) {
		...
		val shake = (mHeaderHeight / 13 * Math.sin(mAppreciation.toDouble())).toInt()
		...
		if (mState == RefreshState.Refreshing || mState == RefreshState.RefreshFinish) {
			...
			mCloudDrawable!!.draw(canvas)
			canvas.rotate(5 * Math.sin((mAppreciation / 2).toDouble()).toFloat(), (width / 2).toFloat(), (mHeaderHeight / 2 - mUmbrellaDrawable!!.height()).toFloat())
			calculateFrame(width)
		}

		val centerY = height - mHeaderHeight / 2 + shake
		val centerYBox = centerY + (mHeaderHeight / 2 - mBoxDrawable!!.height()) - Math.min(mHeaderHeight / 2 - mBoxDrawable!!.height(), DensityUtil.dp2px(mAppreciation * 100))
		mBoxDrawable!!.bounds.offsetTo(width / 2 - mBoxDrawable!!.width() / 2, centerYBox - mBoxDrawable!!.height() / 4)
		mBoxDrawable!!.draw(canvas)

		if (mState == RefreshState.Refreshing || mState == RefreshState.RefreshFinish) {
			val bounds = mUmbrellaDrawable!!.bounds
			val centerYUmbrella = centerY - mHeaderHeight + Math.min(mHeaderHeight, DensityUtil.dp2px(mAppreciation * 100))
	...
	private fun calculateFrame(width: Int) {
		mCloudX1 += DensityUtil.dp2px(9f)
		mCloudX2 += DensityUtil.dp2px(5f)
		mCloudX3 += DensityUtil.dp2px(12f)
