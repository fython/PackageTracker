    ...
    class ViewHolder(itemView: View) : ItemViewHolder<HomeListEmptyViewModel>(itemView) {
        ...
        override fun onBind(item: HomeListEmptyViewModel) {
            textView.setText(when (item.type) {
                HomePackageListAdapter.FILTER_ALL -> R.string.frame_empty_tip_all
                HomePackageListAdapter.FILTER_ON_THE_WAY -> R.string.frame_empty_tip_delivering
                HomePackageListAdapter.FILTER_DELIVERED -> R.string.frame_empty_tip_delivered
                else -> 0
            })
        }
